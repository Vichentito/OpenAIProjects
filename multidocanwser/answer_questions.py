...
if __name__ == '__main__':
    ...
    while True:
        ...
        for result in results:
            prompt = open_file('prompt_answer.txt').replace(
                '<<PASSAGE>>', result['content']).replace('<<QUERY>>', query)
            answer = gpt3_completion(prompt)
            #print('\n\n', answer)
            answers.append(answer)
        # summarize the answers together
        all_answers = '\n'.join(answers)
        chunks = textwrap.wrap(all_answers, 8000)
        final = list()
        for chunk in chunks:
            prompt = open_file('prompt_summary.txt').replace(
                '<<SUMMARY>>', chunk).replace('<<QUESTION>>', query)
